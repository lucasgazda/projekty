...
public class CarList {
	...
	public CarList(String fileName) {
		try {
			...
			String filePath = "G:\\Programowanie\\Git\\Stream\\" + fileName + ".txt";
			...
			while(sc.hasNextLine())
			{
				...
				carList.add(new Car(tab[0], Double.parseDouble(tab[1]), Double.parseDouble(tab[2]), 
						Double.parseDouble(tab[3]), new ArrayList<>(Arrays.asList(tab[4].split(";")))));
			}
			sc.close();
	...
	public void listBrandMilage() {
		try {
			String fileName = "G:\\Programowanie\\Git\\Stream\\listBrandMilage.txt";
	...
	public void statisticPriceCapacityMilage() {
		...
		try {
			String fileName = "G:\\Programowanie\\Git\\Stream\\statisticPriceCapacityMilage.txt";
			PrintWriter pw = new PrintWriter(fileName);
